...
export const WorkflowManager: React.FC<WorkflowManagerProps> = ({
  ...
  // View mode tab: Matrix vs Individual Piece Tracker vs Kanban vs Party Slips
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-xs p-3 flex flex-col md:flex-row items-center justify-between gap-3">
        ...
        <div className="flex items-center flex-wrap gap-2 w-full md:w-auto justify-end">
          ...
          <button
            ...
            className="flex items-center space-x-1.5 px-3 py-2 rounded-xl bg-emerald-50 hover:bg-emerald-100 text-emerald-800 border border-emerald-200 text-xs font-bold transition-colors shadow-2xs cursor-pointer"
            title="Export all production data to Excel (.xlsx) file"
          >
            <Download className="h-3.5 w-3.5 text-emerald-700" />
            <span>Export Excel (.xlsx)</span>
          </button>

          {/* Reset / Clear All Orders Button */}
          {onClearAllOrders && (items.length > 0 || currentOrderSlips.length > 0) && (
            <button
              id="btn-clear-all-orders"
              type="button"
              onClick={onClearAllOrders}
              className="flex items-center space-x-1.5 px-3 py-2 rounded-xl bg-rose-50 hover:bg-rose-100 text-rose-700 border border-rose-200 text-xs font-semibold transition-colors shadow-2xs"
              title="Delete all demo orders and start completely fresh"
            >
