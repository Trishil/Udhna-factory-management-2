  ...
  return (
    <div id="individual-piece-tracker-root" className="space-y-6">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-xs overflow-hidden">
        <div className="p-5 bg-gradient-to-r from-slate-900 via-indigo-950 to-slate-900 text-white flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4">
          <div className="flex items-center space-x-3">
            <div className="p-3 rounded-xl bg-blue-600 shadow-md shadow-blue-500/30">
            ...
            <div>
              <div className="flex items-center space-x-2 flex-wrap">
                ...
                <span className="px-2 py-0.5 rounded-md bg-blue-500/30 border border-blue-400/40 text-blue-200 text-[10px] font-mono font-bold uppercase">
                  Unit-Level Breakdown
                </span>
                <span className="px-2 py-0.5 rounded-md bg-rose-500/30 border border-rose-400/40 text-rose-200 text-[10px] font-mono font-bold uppercase">
          ...
          <div className="flex items-center space-x-2 bg-slate-800/90 p-1 rounded-xl border border-slate-700">
            <button
              type="button"
        ...
        <div className="grid grid-cols-2 sm:grid-cols-5 divide-x divide-y sm:divide-y-0 divide-slate-100 bg-slate-50 border-t border-slate-200 text-center text-xs">
          <div className="p-3">
            <span className="text-[10px] text-slate-500 font-semibold uppercase block">Total Units Tracked</span>
            <span className="text-base font-black text-slate-800 font-mono">{stats.total} Pcs</span>
          </div>

          <div className="p-3">
            <span className="text-[10px] text-slate-500 font-semibold uppercase block">In Embroidery</span>
            <span className="text-base font-black text-purple-700 font-mono">{stats.inEmbroidery} Pcs</span>
          </div>
          ...
          <div className="p-3">
            ...
            <span className="text-base font-black text-blue-700 font-mono">{stats.goodInProduction} Pcs</span>
          </div>
          ...
          <div className="p-3 bg-emerald-50/60">
            ...
            <span className="text-base font-black text-emerald-700 font-mono">{stats.completed} Pcs</span>
          </div>
        </div>
      </div>
      ...
      {/* Action Toast Banner */}
      {lastActionToast && (
        <div className="p-3 bg-slate-900 text-white rounded-xl shadow-lg border border-slate-700 flex items-center justify-between text-xs animate-in slide-in-from-top duration-200">
        ...
        <div className="flex flex-col lg:flex-row items-stretch lg:items-center justify-between gap-3">
          ...
          {orderSlips.length > 0 && (
            <div className="flex items-center space-x-2 flex-1 min-w-[220px]">
              <label htmlFor="slip-filter-select" className="text-xs font-bold text-amber-900 whitespace-nowrap flex items-center space-x-1">
                <FileText className="h-3.5 w-3.5 text-amber-600" />
                <span>Party Slip:</span>
              </label>
              <select
                ...
                className="flex-1 px-3 py-2 rounded-xl border border-amber-300 bg-amber-50/50 text-xs font-bold text-amber-950 focus:ring-2 focus:ring-amber-500 focus:outline-none"
              >
                <option value="all">📋 All Party Slips ({orderSlips.length} Slips)</option>
                {orderSlips.map(s => (
                  <option key={s.id} value={s.id}>
                    {s.partyName} (Job: {s.jobNo}, Ch: {s.chalanNo}) &bull; {s.totalPcs} pcs
                  </option>
                ))}
              </select>
            </div>
          ...
          {/* Job / Lot Selector */}
          <div className="flex items-center space-x-2 flex-1 min-w-[220px]">
            <label htmlFor="lot-filter-select" className="text-xs font-bold text-slate-700 whitespace-nowrap">
              Select Job / Lot:
            </label>
            <select
              ...
              className="flex-1 px-3 py-2 rounded-xl border border-slate-300 bg-white text-xs font-bold text-slate-800 focus:ring-2 focus:ring-blue-500 focus:outline-none"
            >
              <option value="all">⚡ All Active Lots ({items.length} lots)</option>
              {items.map(it => (
                <option key={it.id} value={it.id}>
                  {it.partyName || it.partyOrClientName || 'Client'} &bull; Job: {it.jobNo || it.lotNumber} &bull; {it.fabricType} ({it.fabricColor || 'Color'}) &bull; {it.pieces || it.quantity} Pcs
                </option>
              ))}
            </select>
          </div>
          ...
          <div className="relative flex-1 min-w-[180px]">
            ...
            <input
              type="text"
              placeholder="Search piece tag, #1, defect reason..."
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              className="w-full pl-9 pr-3 py-2 rounded-xl border border-slate-300 text-xs text-slate-800 placeholder-slate-400 focus:ring-2 focus:ring-blue-500 focus:outline-none"
            />
          </div>
          ...
          <div className="flex items-center space-x-2">
            <select
              ...
              className="px-3 py-2 rounded-xl border border-slate-300 bg-white text-xs font-medium text-slate-800 focus:ring-2 focus:ring-blue-500"
            >
            ...
            >
              ...
              <option value="good">Fine / Standard (Good)</option>
              <option value="needs_alter">⚠️ Needs Altering / In Rework</option>
